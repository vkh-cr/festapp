((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_271",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,F,G,H,I,K,B={anT:function anT(d,e,f){this.c=d
this.a=e
this.b=f},
cDU(d,e,f){return new B.a20(d,f,e,null)},
d_n(d,e){},
a20:function a20(d,e,f,g){var _=this
_.c=d
_.d=e
_.r=f
_.a=g},
aMf:function aMf(){var _=this
_.d=$
_.c=_.a=_.f=_.e=null},
c4l:function c4l(d){this.a=d},
c4k:function c4k(d,e){this.a=d
this.b=e},
aUc:function aUc(){},
cDV(d,e){var x=null,w=d===D.aoF?250:0
return new B.a23(d,w,e,new A.fd(x,x,y.n),new B.Cx(x,C.AL,x,!1,!1,C.a_,C.jC,1),$.aa())},
a23:function a23(d,e,f,g,h,i){var _=this
_.y=d
_.z=e
_.as=f
_.ch=g
_.cy=_.cx=_.CW=null
_.db=!1
_.a=h
_.R$=0
_.P$=i
_.b7$=_.bd$=0},
bpj:function bpj(d){this.a=d},
bpk:function bpk(d){this.a=d},
bpl:function bpl(d){this.a=d},
bpm:function bpm(d){this.a=d},
Cx:function Cx(d,e,f,g,h,i,j,k){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j
_.w=k},
azi:function azi(d){this.a=d},
bCM:function bCM(d,e,f,g,h,i,j,k){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j
_.w=k},
bCN:function bCN(){},
q3(){var x=0,w=A.m(y.v)
var $async$q3=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:x=2
return A.d(C.cA.fX("HapticFeedback.vibrate","HapticFeedbackType.lightImpact",y.v),$async$q3)
case 2:return A.k(null,w)}})
return A.l($async$q3,w)}},D
A=c[0]
C=c[2]
E=c[210]
F=c[178]
G=c[136]
H=c[340]
I=c[233]
K=c[197]
B=a.updateHolder(c[36],B)
D=c[228]
B.anT.prototype={
J(){return"DetectionSpeed."+this.b}}
B.a20.prototype={
M(){return new B.aMf()}}
B.aMf.prototype={
bcV(d,e){this.a.toString
return},
B(d){var x=this.d
x===$&&A.b()
return new G.fN(x,new B.c4l(this),null,null,y.i)},
Ze(){var x=0,w=A.m(y.v),v=this,u,t,s
var $async$Ze=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:s=v.a
s.toString
u=v.d
u===$&&A.b()
t=u.ch
v.f=new A.cL(t,A.t(t).h("cL<1>")).Pf(s.d,!1,B.diu())
x=2
return A.d(u.kJ(0),$async$Ze)
case 2:return A.k(null,w)}})
return A.l($async$Ze,w)},
O3(){var x=0,w=A.m(y.v),v=this,u
var $async$O3=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:v.a.toString
u=v.f
u=u==null?null:u.a_(0)
x=2
return A.d(y.w.b(u)?u:A.cD(u,y.v),$async$O3)
case 2:v.f=null
u=v.d
u===$&&A.b()
x=3
return A.d(u.dK(0),$async$O3)
case 3:v.a.toString
return A.k(null,w)}})
return A.l($async$O3,w)},
U(){var x,w=this
w.ah()
x=w.a.c
w.d!==$&&A.b9()
w.d=x
w.Ze()},
l(){this.ag()
this.O3()},
r7(d){var x,w=this.d
w===$&&A.b()
x=w.a
if(x.d){x=x.c
x=(x==null?null:x.a)!==C.ya}else x=!1
if(!x)return
switch(d.a){case 0:case 3:case 4:return
case 1:w.kJ(0)
break
case 2:w.dK(0)
break}}}
B.aUc.prototype={}
B.a23.prototype={
blA(){var x=this
x.CW=$.Fw().gawZ().Pf(new B.bpj(x),!1,new B.bpk(x))
x.cx=$.Fw().gaG3().el(new B.bpl(x))
x.cy=$.Fw().gaHh().el(new B.bpm(x))},
bn1(){var x=this,w=x.a
if(!w.d||!w.e||x.db)return!1
w=x.CW
if(w!=null)w.a_(0)
w=x.cx
if(w!=null)w.a_(0)
w=x.cy
if(w!=null)w.a_(0)
x.cy=x.cx=x.CW=null
w=x.a
x.sp(0,w.bwc(!1,w.r===C.jC?C.jC:C.abO))
return!0},
kJ(d){var x=0,w=A.m(y.v),v,u=2,t=[],s=this,r,q,p,o,n,m,l
var $async$kJ=A.i(function(e,f){if(e===1){t.push(f)
x=u}while(true)switch(x){case 0:if(s.db)throw A.n(A.Cw(C.a3W,A.Cv(null,null,C.a3W.gn4(0))))
if(s.a.e){x=1
break}r=C.AL
q=new B.bCM(r,null,s.y,s.z,s.as,!1,!1,!1)
u=4
s.blA()
x=7
return A.d($.Fw().no(0,q),$async$kJ)
case 7:p=f
if(!s.db)s.sp(0,s.a.bwx(p.b,r,!0,!0,p.c,p.a))
u=2
x=6
break
case 4:u=3
l=t.pop()
m=A.ah(l)
if(m instanceof A.wi){o=m
if(!s.db)s.sp(0,s.a.bwA(C.AL,o,!0,!1,C.a_,C.jC,1))}else throw l
x=6
break
case 3:x=2
break
case 6:case 1:return A.k(v,w)
case 2:return A.j(t.at(-1),w)}})
return A.l($async$kJ,w)},
dK(d){var x=0,w=A.m(y.v),v=this
var $async$dK=A.i(function(e,f){if(e===1)return A.j(f,w)
while(true)switch(x){case 0:x=v.bn1()?2:3
break
case 2:x=4
return A.d($.Fw().dK(0),$async$dK)
case 4:case 3:return A.k(null,w)}})
return A.l($async$dK,w)},
l(){var x=0,w=A.m(y.v),v,u=this
var $async$l=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:if(u.db){x=1
break}u.db=!0
u.ch.av(0)
u.fh()
x=3
return A.d($.Fw().l(),$async$l)
case 3:case 1:return A.k(v,w)}})
return A.l($async$l,w)}}
B.Cx.prototype={
NI(d,e,f,g,h,i,j,k){var x=this,w=d==null?x.a:d,v=e==null?x.b:e,u=g==null?x.d:g,t=h==null?x.e:h,s=i==null?x.f:i,r=j==null?x.r:j
return new B.Cx(w,v,f,u,t,s,r,k==null?x.w:k)},
bwx(d,e,f,g,h,i){return this.NI(d,e,null,f,g,h,i,null)},
bwA(d,e,f,g,h,i,j){return this.NI(null,d,e,f,g,h,i,j)},
bw_(d){var x=null
return this.NI(x,x,x,x,x,x,x,d)},
bvX(d){var x=null
return this.NI(x,x,x,x,x,x,d,x)},
bwc(d,e){var x=null
return this.NI(x,x,x,x,d,x,e,x)}}
B.azi.prototype={
B(d){var x=null,w=A.a([D.bqk],y.u)
w.push(A.U(C.hW.gn4(0),x,x,x,x,x,x,x,I.pz,x,x,x,x,x))
return new A.nz(C.r,A.bT(A.al(w,C.i,C.f,C.Y,x,C.m),x,x),x)}}
B.bCM.prototype={
fC(){var x,w,v=this,u=A.K(y.x,y.q)
u.m(0,"facing",v.a.c)
x=v.e
if(x.length!==0){w=A.Z(x).h("Q<1,f>")
u.m(0,"formats",A.H(new A.Q(x,new B.bCN(),w),!0,w.h("ab.E")))}u.m(0,"returnImage",!1)
u.m(0,"speed",v.c.c)
u.m(0,"timeout",v.d)
u.m(0,"torch",!1)
u.m(0,"useNewCameraSelector",!1)
return u}}
var z=a.updateTypes(["e(L,Cx,e?)","V<~>()","~(G,dN)"])
B.c4l.prototype={
$3(d,e,f){if(!e.d){this.a.a.toString
return D.anA}if(e.c!=null){this.a.a.toString
return new B.azi(null)}return new A.hE(new B.c4k(this.a,e),null)},
$S:z+0}
B.c4k.prototype={
$2(d,e){var x,w,v,u=this.a,t=this.b
u.bcV(t,e)
u=u.a
u.toString
x=t.f
t=A.a0(1/0,e.a,e.b)
w=A.a0(1/0,e.c,e.d)
v=A.lU(K.uH(F.ap5(C.O,new A.ar(x.a,x.b,$.Fw().axb(),null),C.k,u.r),new A.T(t,w)),C.A,null)
return v},
$S:91}
B.bpj.prototype={
$1(d){var x=this.a.ch
if((x.c&4)!==0||d==null)return
x.t(0,d)},
$S:1000}
B.bpk.prototype={
$1(d){var x=this.a.ch
if((x.c&4)!==0)return
x.kc(d)},
$S:93}
B.bpl.prototype={
$1(d){var x=this.a
if(x.db)return
x.sp(0,x.a.bvX(d))},
$S:1001}
B.bpm.prototype={
$1(d){var x=this.a
if(x.db)return
x.sp(0,x.a.bw_(d))},
$S:118}
B.bCN.prototype={
$1(d){return d.c},
$S:1002};(function installTearOffs(){var x=a._static_2,w=a._instance_0u
x(B,"diu","d_n",2)
w(B.a23.prototype,"gfa","l",1)})();(function inheritance(){var x=a.mixin,w=a.inherit,v=a.inheritMany
w(B.anT,A.e7)
w(B.a20,A.I)
w(B.aUc,A.M)
w(B.aMf,B.aUc)
v(A.bI,[B.c4l,B.bpj,B.bpk,B.bpl,B.bpm,B.bCN])
w(B.c4k,A.dr)
w(B.a23,A.bO)
v(A.G,[B.Cx,B.bCM])
w(B.azi,A.a9)
x(B.aUc,A.eA)})()
A.br(b.typeUniverse,JSON.parse('{"a20":{"I":[],"e":[]},"aMf":{"M":["a20"],"eA":[]},"a23":{"bO":["Cx"],"at":[]},"azi":{"a9":[],"e":[]}}'))
var y={w:A.B("V<~>"),u:A.B("u<e>"),x:A.B("h"),i:A.B("fN<Cx>"),n:A.B("fd<lR>"),q:A.B("G?"),v:A.B("~")};(function constants(){var x=a.makeConstList
D.anA=new A.nz(C.r,null,null)
D.KB=new B.anT(0,0,"noDuplicates")
D.aoF=new B.anT(1,1,"normal")
D.L1=new A.ap(0,24,0,12)
D.LX=new A.aB(57657,"MaterialIcons",null,!1)
D.bMd=A.a(x([]),A.B("u<jj>"))
D.amB=new A.W(1,0.5098039215686274,0.6941176470588235,1,C.j)
D.amu=new A.W(1,0.26666666666666666,0.5411764705882353,1,C.j)
D.am5=new A.W(1,0.1607843137254902,0.4745098039215686,1,C.j)
D.anf=new A.W(1,0.1607843137254902,0.3843137254901961,1,C.j)
D.aVa=new A.c([100,D.amB,200,D.amu,400,D.am5,700,D.anf],A.B("c<f,W>"))
D.a3K=new A.I6(D.aVa,1,0.26666666666666666,0.5411764705882353,1,C.j)
D.atH=new A.da(E.rn,null,C.p,null,null)
D.bqk=new A.a5(H.BD,D.atH,null)
D.abj=new A.a1(!0,C.r,null,null,null,null,16,C.a6,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_271",e:"endPart",h:b})})($__dart_deferred_initializers__,"4AN2P+Sctgd6WMuXjGUIXRPeokU=");