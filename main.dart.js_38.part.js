((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_38",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,A={
avi(d,e,f){return new A.avh(e,f,d,null)},
avh:function avh(d,e,f,g){var _=this
_.d=d
_.e=e
_.f=f
_.a=g}},D,E
B=c[0]
C=c[2]
A=a.updateHolder(c[108],A)
D=c[151]
E=c[141]
A.avh.prototype={
B(d){var y,x=null,w=B.C(d).ax.a===C.r||this.f===!0?"assets/icons/fstapplogo.dark.svg":"assets/icons/fstapplogo.svg",v=this.d,u=this.e
if(C.e.ld(w.toLowerCase(),".svg"))y=new D.Lu(u,v,C.h6,new D.TT(w,x,x,x,x),x,!1,"Festapp logo",!1,x,x)
else y=E.a1N(w,x,v,x,u)
return B.iI(!1,x,!0,y,x,!0,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x)}}
var z=a.updateTypes([]);(function inheritance(){var y=a.inherit
y(A.avh,B.aa)})()
B.c4(b.typeUniverse,JSON.parse('{"avh":{"aa":[],"e":[]}}'))};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_38",e:"endPart",h:b})})($__dart_deferred_initializers__,"UofgeHL5onX1JM/iybEwGGViEyQ=");