...
| `send-sign-in-code` | Resets password and sends sign-in code. Used by admins/editors to invite users. |
| `send-reset-password-link` | Self-service "Forgot Password" flow. Sends reset email with branding from org config. |
| `send-ticket-order` | Creates order via RPC, generates QR payment codes, optional Fakturoid invoicing, sends confirmation email. |
