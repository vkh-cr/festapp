...
| CSM-specific data jsou identifikovatelná: offline-map manifests, legal sources a recovery SQL; store metadata/screenshots vlastní private `festapp-release-internal`. | `automation/offline-map/manifests/csmostrava2026`, `automation/release/legal`, `database/recovery/2026-*csmostrava*`, `festapp-release-internal/store-listings/csm-ostrava-2026` | Tyto artefakty zůstávají na explicitní tenant boundary, nepromíchají se se shared code. |
