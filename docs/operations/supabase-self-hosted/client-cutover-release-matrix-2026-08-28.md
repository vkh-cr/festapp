...
| AKSM / CSA 2024 (`prod/aksmcz`) | `festapp.aksmcz` / observed through build `270` on several legacy refs | `fstapp.csa` / `>230` | Historical 12–17 August 2024 event is preserved in canonical organization `4`; old Netlify remains reachable but the recorded custom domain is dead. Do not build until store readback chooses archive/retirement. |
