...
| Celostátní setkání animátorů / CSA 2024 (`prod/aksmcz`) | listed historical version only; no new release | public listing 404; no new release | Web-only lane at `csa2024.festapp.net`, without OneSignal. Canonical organization is `4`; retire old Netlify through a path/query-preserving redirect. App Store removal from sale remains a separate store operation. |
