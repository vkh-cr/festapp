((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_211",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={
cbQ(d){var x=null,w=A.z(["id",null],y.g,y.b)
return new B.aop("MapRoute",new A.B3(x,d,x),w,C.bv,x,"",x)},
aop:function aop(d,e,f,g,h,i,j){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j}},D
A=c[0]
C=c[2]
B=a.updateHolder(c[56],B)
D=c[259]
B.aop.prototype={}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(B.aop,A.j9)})()
A.bD(b.typeUniverse,JSON.parse('{"aop":{"j9":["B3"]}}'))
var y={g:A.w("f"),b:A.w("@")};(function constants(){D.yT=new A.aQ(58172,"MaterialIcons",null,!1)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_211",e:"endPart",h:b})})($__dart_deferred_initializers__,"jyoeRRw+DZ52hqgRrAp39G722KQ=");