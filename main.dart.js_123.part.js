((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_123",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,D={
aFr(d){if(!A.ff(d,!1).f.Ib())C.hq(d,"",y.a)
else A.ff(d,!1).f.gEU().y.back()}},A
B=c[0]
C=c[160]
D=a.updateHolder(c[96],D)
A=c[163]
var z=a.updateTypes([])
var y={a:B.F("H?")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_123",e:"endPart",h:b})})($__dart_deferred_initializers__,"J/NClk06Ex7RxsbDAAUHqUVuKSQ=");