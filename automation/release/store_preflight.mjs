...
const manifest = JSON.parse(fs.readFileSync(path.join(root, 'automation/release/app_store_cutover.json'), 'utf8'));
const errors = [];
const warnings = [];
...
for (const [field, expected] of [['privacy_url', manifest.urls.privacy], ['support_url', manifest.urls.support], ['marketing_url', manifest.urls.marketing]]) {
...
for (const [device, rule] of Object.entries(manifest.screenshots)) {
  const directory = path.join(screenshotRoot, device);
  const files = fs.existsSync(directory) ? fs.readdirSync(directory).filter((file) => file.endsWith('.png')).sort() : [];
  ...
  for (const name of files) {
    try {
      ...
      if (info.width !== rule.width || info.height !== rule.height) fail(`${device}/${name} has ${info.width}x${info.height}`);
      if ([4, 6].includes(info.colorType)) fail(`${device}/${name} contains alpha`);
    } catch (error) { fail(`${device}/${name}: ${error.message}`); }
  }
}
...
if (/deleteAccountInstructions|credentials to info@festapp\.net|přihlašovacími údaji na info@festapp\.net/.test(
  runtimeFiles.map((file) => fs.readFileSync(path.join(root, file), 'utf8')).join('\n')
)) fail('legacy credential-by-email deletion copy remains');

warnings.push('Legal/DSA/reviewer/TestFlight/production gates require external evidence.');
