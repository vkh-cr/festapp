...
const projectConfig = fs.readFileSync(path.join(root, 'automation/project.conf'), 'utf8');
const appName = projectConfig.match(/^APP_NAME=(?:"([^"]*)"|(.*))$/m)?.slice(1).find((value) => value !== undefined)?.trim();
if (!appName) throw new Error('automation/project.conf lacks APP_NAME');
