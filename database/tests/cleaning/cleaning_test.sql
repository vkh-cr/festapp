...
-- ---------------------------------------------------------------------------
-- 10. Quality reviews (feature C): a participant rates a toilet (upsert = one
--     row per user/place), re-rating overwrites; get_cleaning_review returns the
--     caller's rating + aggregate; get_cleaning_status carries rating_avg/count.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
    v_oc   bigint := (SELECT v FROM _cln WHERE k = 'occasion');
    ...
    v_wc2_node jsonb;
BEGIN
    -- Outsider (not blocked) rates WC 02 happy(3), then re-rates sad(1).
    PERFORM set_config('request.jwt.claim.sub', get_user_id('cln_outsider')::text, true);
    v_res := public.submit_cleaning_review(v_wc2, 3, 'clean enough');
    PERFORM assert_eq(v_res->>'code', '200', 'submit review → 200');

    v_res := public.get_cleaning_review(v_wc2);
    PERFORM assert_eq(v_res->'data'->>'my_rating', '3', 'get_cleaning_review returns my rating');

    v_res := public.submit_cleaning_review(v_wc2, 1, NULL);
    PERFORM assert_eq(v_res->>'code', '200', 're-rate → 200 (upsert)');

    SELECT count(*) INTO v_rows FROM public.cleaning_reviews
     WHERE place = v_wc2 AND created_by = get_user_id('cln_outsider');
    PERFORM assert_eq(v_rows::text, '1', 're-rating upserts (still one row per user/place)');

    v_res := public.get_cleaning_review(v_wc2);
    PERFORM assert_eq(v_res->'data'->>'my_rating', '1', 'get_cleaning_review reflects the new rating');

    -- Crew also rates WC 02 happy(3): aggregate avg = (1+3)/2 = 2.00, count = 2.
    PERFORM set_config('request.jwt.claim.sub', get_user_id('cln_crew')::text, true);
    PERFORM public.submit_cleaning_review(v_wc2, 3, NULL);

    v_status := public.get_cleaning_status(v_oc)->'data';
    SELECT elem INTO v_wc2_node
      FROM jsonb_array_elements(v_status) elem WHERE (elem->>'place')::bigint = v_wc2;
    PERFORM assert_eq(v_wc2_node->>'rating_count', '2', 'status carries rating_count');
    PERFORM assert_eq((v_wc2_node->>'rating_avg')::numeric::text, '2.00', 'status carries rating_avg');

    RAISE NOTICE 'test 10 (quality reviews) passed';
END $$ LANGUAGE plpgsql;

-- ---------------------------------------------------------------------------
-- 11. Review validation: invalid rating → 400; non-toilet → 400; a blocked
--     participant cannot rate → 403 (reporter is blocked from test 9).
-- ---------------------------------------------------------------------------
DO $$
DECLARE
    v_wc2  bigint := (SELECT v FROM _cln WHERE k = 'wc2');
...
BEGIN
    PERFORM set_config('request.jwt.claim.sub', get_user_id('cln_outsider')::text, true);
    v_res := public.submit_cleaning_review(v_wc2, 5, NULL);
    PERFORM assert_eq(v_res->>'code', '400', 'rating out of range → 400');

    v_res := public.submit_cleaning_review(v_room, 3, NULL);
    PERFORM assert_eq(v_res->>'code', '400', 'review on non-toilet → 400');

    -- cln_reporter is blocked (test 9) → cannot rate.
    PERFORM set_config('request.jwt.claim.sub', get_user_id('cln_reporter')::text, true);
    v_res := public.submit_cleaning_review(v_wc2, 3, NULL);
    PERFORM assert_eq(v_res->>'code', '403', 'blocked participant cannot rate → 403');

    RAISE NOTICE 'test 11 (review validation) passed';
END $$ LANGUAGE plpgsql;
