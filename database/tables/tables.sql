...
-- Cleaning quality reviews: one smiley rating (1 sad / 2 neutral / 3 happy) per
-- participant per toilet (upsert). Written only via submit_cleaning_review.
create table if not exists public.cleaning_reviews (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  CONSTRAINT cleaning_reviews_pkey PRIMARY KEY (id),
  CONSTRAINT cleaning_reviews_occasion_fkey FOREIGN KEY (occasion) REFERENCES public.occasions (id),
  CONSTRAINT cleaning_reviews_place_fkey FOREIGN KEY (place) REFERENCES public.places (id),
  CONSTRAINT cleaning_reviews_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.user_info (id),
  CONSTRAINT cleaning_reviews_rating_check CHECK (rating BETWEEN 1 AND 3),
  CONSTRAINT cleaning_reviews_place_user_unique UNIQUE (place, created_by)
) TABLESPACE pg_default;
