...
class TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final TextPainter textPainter = TextPainter(textDirection: TextDirection.ltr, textAlign: TextAlign.center);
    ...
    final Color dayBgColor1 = isDarkMode ? Colors.grey.shade800.withOpacity(0.15) : Colors.grey.shade200.withOpacity(0.3);
    final Color dayBgColor2 = isDarkMode ? Colors.grey.shade800.withOpacity(0.35) : Colors.grey.shade200.withOpacity(0.7);
    ...
    while (currentDayIter.isBefore(end)) {
      ...
      final dayNameStyle = TextStyle(color: primaryTextColor, fontSize: 13, fontWeight: FontWeight.bold, height: 1.0);
      final dayNumberStyle = TextStyle(color: secondaryTextColor, fontSize: 10, fontWeight: FontWeight.normal);

      String dayNameStr = weekdays[currentDayIter.weekday - 1];
      String dateStr = "${currentDayIter.day.toString().padLeft(2, '0')}/${currentDayIter.month.toString().padLeft(2, '0')}";

      textPainter.text = TextSpan(
        children: [
          TextSpan(text: dayNameStr.toUpperCase(), style: dayNameStyle),         // Day name on top
      ...
      if (logicalDayWidth >= labelWidth) {
        ...
        if (logicalDayWidth < repeatInterval) {
          ...
          for (double x = startX; x < iterDayEndX - labelWidth; x += repeatInterval) {
            if (x + labelWidth > iterDayStartX) {
              textPainter.paint(canvas, Offset(x, yPosLabel));
            }
          }
          for (double x = startX - repeatInterval; x > iterDayStartX; x -= repeatInterval) {
    ...
    while (currentTickTime.isBefore(end) || currentTickTime.isAtSameMomentAs(end)) {
      ...
      if (dx >= calculatedFullLogicalWidth + 1 && calculatedFullLogicalWidth > 0) break;
      ...
      if (drawLabel) {
        ...
        final timeTextPainter = TextPainter(text: TextSpan(text: labelStr, style: TextStyle(color: (isHourTick && currentTickTime.hour == 0) ? primaryTextColor : secondaryTextColor, fontSize: labelFontSize.toDouble())), textDirection: TextDirection.ltr);
        timeTextPainter.layout();

        double labelTopY = mainLineY - actualTickHeight - timeTextPainter.height - textVerticalPadding;
        double labelXPos = dx - timeTextPainter.width / 2;

        if (labelTopY < timeTicksAreaTopY) labelTopY = timeTicksAreaTopY + textVerticalPadding;

        if (labelXPos + timeTextPainter.width >= -1 && labelXPos <= calculatedFullLogicalWidth +1) {
