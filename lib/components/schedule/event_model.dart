...
class EventModel extends ITrinaRowModel {
  ...
  factory EventModel.fromJson(Map<String, dynamic> json) {
    var eventGroups =
        json.containsKey(eventGroupsTable) && json[eventGroupsTable] != null
            ? json[eventGroupsTable]
            : null;
    ...
    if (json.containsKey(childEventsList)) {
      ...
      childEventsIds.addAll(toAdd.map((e) {
        return e as int;
      }));
    }
    ...
    if (json.containsKey(Tb.event_roles.table)) {
      ...
      eventRoles.addAll(toAdd.map((e) {
        return e[Tb.event_roles.role] as int;
      }));
    }

    Map<String, dynamic>? eventData = json.containsKey(dataColumn)
        ? json[dataColumn] as Map<String, dynamic>?
        : null;
    ...
    return EventModel(
      startTime: json.containsKey(startTimeColumn)
          ? DateTime.parse(json[startTimeColumn]).toOccasionTime()
          : DateTime.fromMicrosecondsSinceEpoch(0),
      endTime: json.containsKey(endTimeColumn)
          ? DateTime.parse(json[endTimeColumn]).toOccasionTime()
          : DateTime.fromMicrosecondsSinceEpoch(0),
      id: json[idColumn],
      isHidden: json.containsKey(isHiddenColumn) ? json[isHiddenColumn] : false,
      updatedAt: json[updatedAtColumn] != null
          ? DateTime.parse(json[updatedAtColumn])
          : null,
      title: json.containsKey(titleColumn) ? json[titleColumn] : null,
      description:
          json.containsKey(descriptionColumn) ? json[descriptionColumn] : null,
      ...
      splitForMenWomen: json.containsKey(splitForMenWomenColumn)
          ? json[splitForMenWomenColumn]
          : false,
      isSignedIn:
          json.containsKey(isSignedInColumn) ? json[isSignedInColumn] : false,
      isGroupEvent: json.containsKey(isGroupEventColumn)
          ? json[isGroupEventColumn]
          : false,
      isInMySchedule: json.containsKey(isEventInMyProgramColumn)
          ? json[isEventInMyProgramColumn]
          : false,
      childEventIds: childEventsIds,
      parentEventIds: parentEventsIds,
      eventRolesIds: eventRoles,
      currentParticipants: json.containsKey(eventUsersTable)
          ? json[eventUsersTable][0]["count"]
          : json.containsKey(currentParticipantsColumn)
              ? json[currentParticipantsColumn]
              : null,
      currentUsersSaved: json[Tb.event_users_saved.table] != null
          ? json[Tb.event_users_saved.table][0]["count"]
          : null,
      occasionId: json.containsKey(Tb.events.occasion)
          ? json[Tb.events.occasion]
          : null,
  ...
  bool isFull() =>
      ...
      currentParticipants! >= maxParticipants!;
  static bool isEventSupportingSignIn(EventModel? event) =>
      event != null &&
  ...
  static EventModel fromPlutoJson(Map<String, dynamic> json) {
    ...
    return EventModel(
      ...
      place: placeId == null
          ? null
          : PlaceModel(id: placeId, title: "", description: "", type: ""),
      type: json[Tb.events.type],
  ...
  TrinaRow toTrinaRow(BuildContext context) {
    return TrinaRow(cells: {
      ...
      startDateColumn:
          TrinaCell(value: DateFormat('yyyy-MM-dd').format(startTime)),
      ...
      placeColumn: TrinaCell(
          value: place == null
              ? PlaceModel.WithoutValue
              : place!.toPlutoSelectString()),
      ...
      parentEventColumn: TrinaCell(
          value: parentEventIds?.map((e) => e.toString()).join(",") ?? ""),
      Tb.event_roles.role: TrinaCell(
          value: eventRolesIds?.map((e) => e.toString()).join(",") ?? ""),
      Tb.event_users.table: TrinaCell(
          value: (maxParticipants ?? 0) > 0
  ...
  String toBasicString() =>
      "$title${isCancelled ? ' (${CommonStrings.cancelled})' : ''}";
  ...
  static HashSet<EventModel> CreateEventModelSet() {
    return HashSet<EventModel>(
        equals: (a, b) => a.id == b.id, hashCode: (a) => a.id.hashCode);
  }
}
