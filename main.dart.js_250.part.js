((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_250",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,E,D,F,B={
d7I(d,e,f,g){throw A.n(A.aH("Process.runSync"))},
aCE:function aCE(){},
aqf:function aqf(){this.c=this.b=$},
bw1:function bw1(){},
bw2:function bw2(){},
bBt:function bBt(d,e){this.a=d
this.b=e},
Rk:function Rk(d,e,f,g){var _=this
_.c=d
_.d=e
_.a=f
_.b=g},
bvH:function bvH(d){this.a=d},
bvI:function bvI(d){this.a=d},
bPS:function bPS(){},
doG(d){var w,v=null
try{v=C.ajT.bP2("xdg-user-dir",A.a([d],x.x),E.as)}catch(w){throw w}return D.bak(v.gbR5().od(0,"\n").h(0,0))},
bXH:function bXH(){},
Q_(){var w=0,v=A.k(x.w),u,t=2,s=[],r,q,p,o,n,m,l,k,j,i,h
var $async$Q_=A.f(function(d,e){if(d===1){s.push(e)
w=t}while(true)switch(w){case 0:j=null
t=4
w=$.cUc()||$.cUb()?7:9
break
case 7:h=J
w=10
return A.d(B.ctw(),$async$Q_)
case 10:j=h.cvA(e)
w=8
break
case 9:w=$.cUe()?11:13
break
case 11:w=14
return A.d(B.ctA(),$async$Q_)
case 14:o=e
j=o==null?null:J.cvA(o)
w=12
break
case 13:w=$.cCO()?15:17
break
case 15:o=$.cv1()
n=new B.bvI(new B.bPS())
$.Ng().m(0,n,o)
r=n
w=18
return A.d(r.aKC(),$async$Q_)
case 18:j=e
w=16
break
case 17:w=$.cUd()?19:20
break
case 19:o=D.dfn()
m=$.cv1()
l=new B.bvH(o)
$.Ng().m(0,l,m)
q=l
m=B.doG("DOWNLOAD")
o=m==null?null:m.a
w=21
return A.d(A.dG(o,x.w),$async$Q_)
case 21:j=e
case 20:case 16:case 12:case 8:t=2
w=6
break
case 4:t=3
i=s.pop()
o=A.ag(i)
if(x.g.b(o)){p=o
J.ao(p)
throw i}else throw i
w=6
break
case 3:w=2
break
case 6:u=j
w=1
break
case 1:return A.i(u,v)
case 2:return A.h(s.at(-1),v)}})
return A.j($async$Q_,v)},
d3N(d){if(E.e.n(d,"."))return d
else{if(d.length!==0)return"."+d
return""}},
cxp(d,e,f,g,h){var w=0,v=A.k(x.o),u
var $async$cxp=A.f(function(i,j){if(i===1)return A.h(j,v)
while(true)switch(w){case 0:u=A.cN("Either filePath or link or file must be provided")
throw A.n(u)
return A.i(null,v)}})
return A.j($async$cxp,v)},
ctw(){var w=0,v=A.k(x.D),u,t
var $async$ctw=A.f(function(d,e){if(d===1)return A.h(e,v)
while(true)switch(w){case 0:$.cCL()
w=3
return A.d(F.a4d.dE("getApplicationDocumentsDirectory",null,!1,x.v),$async$ctw)
case 3:t=e
if(t==null)throw A.n(D.d5R("Unable to get application documents directory"))
u=D.bak(t)
w=1
break
case 1:return A.i(u,v)}})
return A.j($async$ctw,v)},
ctA(){var w=0,v=A.k(x.i),u,t
var $async$ctA=A.f(function(d,e){if(d===1)return A.h(e,v)
while(true)switch(w){case 0:$.cCL()
if(!F.aiL.gbH5())A.a7(A.aH("Functionality only available on macOS"))
w=3
return A.d(F.a4d.dE("getDownloadsDirectory",null,!1,x.v),$async$ctA)
case 3:t=e
if(t==null){u=null
w=1
break}u=D.bak(t)
w=1
break
case 1:return A.i(u,v)}})
return A.j($async$ctA,v)}},C
J=c[1]
A=c[0]
E=c[2]
D=c[163]
F=c[377]
B=a.updateHolder(c[89],B)
C=c[319]
B.aCE.prototype={
gcX(d){return"system"},
kJ(d){return this.glR().cf(d)},
glR(){$.Gj()
return E.bH},
gtN(){$.Gj()
return E.ia}}
B.aqf.prototype={
AZ(d,e,f,g){var w=null,v=null,u=null
return this.aLD(d,e,f,g)},
aLD(d,e,f,a0){var w=0,v=A.k(x.v),u,t=2,s=[],r=this,q,p,o,n,m,l,k,j,i,h,g
var $async$AZ=A.f(function(a1,a2){if(a1===1){s.push(a2)
w=t}while(true)switch(w){case 0:j=null
i=null
h=null
d=d
if(f===C.bmD&&j==null)throw A.n(A.cN("customMimeType is required when mimeType is MimeType.custom"))
q=B.d3N(e)
p=i!=null||h!=null
w=!p?3:4
break
case 3:o=d
w=o==null?5:7
break
case 5:w=8
return A.d(B.cxp(null,i,h,null,null),$async$AZ)
case 8:w=6
break
case 7:a2=o
case 6:d=a2
case 4:t=10
w=p?13:15
break
case 13:n=i
if(n==null){n=h
n.toString
n=D.cx5(n)}w=16
return A.d(r.Kr(q,n,f,a0),$async$AZ)
case 16:n=a2
m=r.b=n==null?y.a:n
w=14
break
case 15:n=d
n.toString
l=f.d
if(l.length===0){l=j
l.toString}l=new B.bBt(new A.aqe(a0,n,q,l),new B.bw2())
r.c=l
w=17
return A.d(l.er(0),$async$AZ)
case 17:l=a2
m=r.b=l==null?y.a:l
case 14:u=m
w=1
break
t=2
w=12
break
case 10:t=9
g=s.pop()
throw g
w=12
break
case 9:w=2
break
case 12:case 1:return A.i(u,v)
case 2:return A.h(s.at(-1),v)}})
return A.j($async$AZ,v)},
Kr(d,e,f,g){return this.aLF(d,e,f,g)},
aLF(d,e,f,g){var w=0,v=A.k(x.w),u,t=2,s=[],r,q,p,o,n
var $async$Kr=A.f(function(h,i){if(h===1){s.push(i)
w=t}while(true)switch(w){case 0:t=4
w=7
return A.d(B.Q_(),$async$Kr)
case 7:r=i
n=J
w=8
return A.d(e.aaW(A.o(r)+"/"+g+d),$async$Kr)
case 8:q=n.cvA(i)
u=q
w=1
break
t=2
w=6
break
case 4:t=3
o=s.pop()
throw o
w=6
break
case 3:w=2
break
case 6:case 1:return A.i(u,v)
case 2:return A.h(s.at(-1),v)}})
return A.j($async$Kr,v)}}
B.bw1.prototype={}
B.bw2.prototype={
S3(d){return this.aLE(d)},
aLE(d){var w=0,v=A.k(x.w),u
var $async$S3=A.f(function(e,f){if(e===1)return A.h(f,v)
while(true)switch(w){case 0:w=3
return A.d(A.be3(d),$async$S3)
case 3:if(f){u="Downloads"
w=1
break}u=null
w=1
break
case 1:return A.i(u,v)}})
return A.j($async$S3,v)}}
B.bBt.prototype={
er(d){var w=0,v=A.k(x.w),u,t=this
var $async$er=A.f(function(e,f){if(e===1)return A.h(f,v)
while(true)switch(w){case 0:w=3
return A.d(t.b.S3(t.a),$async$er)
case 3:u=f
w=1
break
case 1:return A.i(u,v)}})
return A.j($async$er,v)}}
B.Rk.prototype={
J(){return"MimeType."+this.b}}
B.bvH.prototype={}
B.bvI.prototype={}
B.bPS.prototype={}
B.bXH.prototype={
bP2(d,e,f){return B.d7I(d,e,C.ajg,f)}}
var z=a.updateTypes([]);(function inheritance(){var w=a.inherit,v=a.inheritMany
w(B.aCE,A.p2)
v(A.E,[B.aqf,B.bw1,B.bBt,B.bPS,B.bXH])
w(B.bw2,B.bw1)
w(B.Rk,A.dY)
v(D.axT,[B.bvH,B.bvI])})()
A.bk(b.typeUniverse,JSON.parse('{"aCE":{"p2":[],"oV":["m","C<l>"]}}'))
var y={a:"Something went wrong, please report the issue https://www.github.com/incrediblezayed/file_saver/issues"}
var x={D:A.A("Hm"),g:A.A("b7"),x:A.A("r<m>"),v:A.A("m"),o:A.A("dv"),i:A.A("Hm?"),w:A.A("m?")};(function constants(){C.ajg=new B.aCE()
C.ajT=new B.bXH()
C.et=new A.ap(10,0,10,0)
C.bOT=new B.Rk("Other","application/octet-stream",20,"other")
C.bmD=new B.Rk("Custom","",31,"custom")})();(function lazyInitializers(){var w=a.lazyFinal
w($,"dw7","cUd",()=>{$.Gj()
return!1})
w($,"dw8","cUe",()=>{$.Gj()
return!1})})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_250",e:"endPart",h:b})})($__dart_deferred_initializers__,"1+zXldr/wvmy7TuaGgcz0F0MHts=");