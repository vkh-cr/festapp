((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_87",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,C,A,B={
cO0(d){return new B.fE(D.C1,null,null,null,d.i("fE<0>"))},
Pt:function Pt(d,e){this.a=d
this.b=e},
fE:function fE(d,e,f,g,h){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.$ti=h}},D
J=c[1]
C=c[0]
A=c[2]
B=a.updateHolder(c[167],B)
D=c[321]
B.Pt.prototype={
I(){return"ConnectionState."+this.b}}
B.fE.prototype={
gbSO(){var y,x=this.b
if(x!=null)return x
x=this.c
if(x!=null){y=this.d
y.toString
C.bgC(x,y)}throw C.n(C.ad("Snapshot has neither data nor error"))},
j(d){var y=this
return"AsyncSnapshot("+y.a.j(0)+", "+C.o(y.b)+", "+C.o(y.c)+", "+C.o(y.d)+")"},
k(d,e){var y=this
if(e==null)return!1
if(y===e)return!0
return y.$ti.b(e)&&e.a===y.a&&J.p(e.b,y.b)&&J.p(e.c,y.c)&&e.d==y.d},
gv(d){return C.af(this.a,this.b,this.c,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a)}}
var z=a.updateTypes([]);(function inheritance(){var y=a.inherit
y(B.Pt,C.eQ)
y(B.fE,C.G)})();(function constants(){D.C1=new B.Pt(0,"none")
D.C2=new B.Pt(1,"waiting")
D.or=new B.Pt(3,"done")})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_87",e:"endPart",h:b})})($__dart_deferred_initializers__,"CjZBc13ckaPTfvP2ods61X6tfOE=");