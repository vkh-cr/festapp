((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_284",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,B={a20:function a20(d,e){this.a=d
this.b=e},
dcc(d){C.b.e2(d,new B.bb3())
return d},
zi(d){return B.dcg(d)},
dcg(d){var w=0,v=A.l(x.l),u,t,s,r
var $async$zi=A.h(function(e,f){if(e===1)return A.i(f,v)
while(true)switch(w){case 0:w=d!=null?3:5
break
case 3:t=$.wx().c1("information").ir(0,y.d)
s=A.da()
s.toString
w=6
return A.d(t.dc("occasion",s).aee(0,"type","eq",d),$async$zi)
case 6:r=f
w=4
break
case 5:t=$.wx().c1("information").ir(0,y.d)
s=A.da()
s.toString
w=7
return A.d(t.dc("occasion",s).aIm("type.eq.,type.is.null"),$async$zi)
case 7:r=f
case 4:u=B.dcc(A.b0(J.bt(r,new B.bbc(),x.b),!0,x.c))
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$zi,v)},
bb3:function bb3(){},
bbc:function bbc(){},
bV3(d,e){var w,v,u,t,s,r,q,p=A.bz("\\d+|\\D+",!0,!1,!1),o=p.vf(0,d),n=x.g
o=A.md(o,new B.bV4(),A.t(o).i("y.E"),n)
w=A.E(o,A.t(o).i("y.E"))
o=p.vf(0,e)
n=A.md(o,new B.bV5(),A.t(o).i("y.E"),n)
v=A.E(n,A.t(n).i("y.E"))
u=0
while(!0){o=w.length
if(!(u<o&&u<v.length))break
t=w[u]
s=v[u]
o=A.bz("^\\d+$",!0,!1,!1)
if(o.b.test(t)){o=A.bz("^\\d+$",!0,!1,!1)
o=o.b.test(s)}else o=!1
if(o){r=C.c.bn(A.dq(t,null),A.dq(s,null))
if(r!==0)return r}else{if(t===s)q=0
else q=t<s?-1:1
if(q!==0)return q}++u}return C.c.bn(o,v.length)},
bV4:function bV4(){},
bV5:function bV5(){},
ar_(){var w=0,v=A.l(x.i),u,t,s,r,q,p
var $async$ar_=A.h(function(d,e){if(d===1)return A.i(e,v)
while(true)switch(w){case 0:q=$.un().c1("occasions").ir(0,"data")
p=A.da()
p.toString
w=3
return A.d(q.dc("id",p).QY(),$async$ar_)
case 3:t=e
if(t!=null){s=J.v(t,"data")
if(s!=null&&J.v(s,"game")!=null){q=J.v(s,"game")
p=J.a1(q)
r=p.h(q,"start")!=null?A.et(p.h(q,"start")):null
u=new B.a20(r,p.h(q,"end")!=null?A.et(p.h(q,"end")):null)
w=1
break}}u=null
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$ar_,v)}}
J=c[1]
A=c[0]
C=c[2]
B=a.updateHolder(c[58],B)
B.a20.prototype={
bf(){var w,v=this.a
v=v==null?null:v.fK()
w=this.b
return A.z(["start",v,"end",w==null?null:w.fK()],x.g,x.b)}}
var z=a.updateTypes(["X<B<eG>>([m?])"])
B.bb3.prototype={
$2(d,e){var w,v,u=d.w
if(u==null)u=0
w=e.w
v=C.c.bn(u,w==null?0:w)
if(v!==0)return v
u=d.d
u=u==null?null:u.toLowerCase()
if(u==null)u=""
w=e.d
w=w==null?null:w.toLowerCase()
return B.bV3(u,w==null?"":w)},
$S:204}
B.bbc.prototype={
$1(d){return A.avn(d)},
$S:123}
B.bV4.prototype={
$1(d){var w=d.b[0]
w.toString
return w},
$S:269}
B.bV5.prototype={
$1(d){var w=d.b[0]
w.toString
return w},
$S:269};(function installTearOffs(){var w=a.installStaticTearOff
w(B,"dB4",0,function(){return[null]},["$1","$0"],["zi",function(){return B.zi(null)}],0,0)})();(function inheritance(){var w=a.inherit,v=a.inheritMany
w(B.a20,A.G)
w(B.bb3,A.eg)
v(A.c8,[B.bbc,B.bV4,B.bV5])})()
var y={d:"id,occasion,created_at,updated_at,is_hidden,title,description,order,type,data,information_hidden(*)"}
var x={c:A.D("eG"),l:A.D("B<eG>"),g:A.D("m"),b:A.D("@"),i:A.D("a20?")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_284",e:"endPart",h:b})})($__dart_deferred_initializers__,"n16Fvb+jfdIPcZYTLXwql3Et3+0=");