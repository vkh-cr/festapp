((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_328",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,N,O,P,Q,L,R,S,D,T,U,M,V,F,H,B={
cYv(d,e,f){return new B.BI(d,f,e)},
BI:function BI(d,e,f){this.c=d
this.d=e
this.a=f},
ab5:function ab5(d){var _=this
_.e=null
_.x=_.w=_.r=_.f=!1
_.y=null
_.z=0
_.Q=d
_.as=$
_.c=_.a=null},
c_V:function c_V(d){this.a=d},
c_S:function c_S(d){this.a=d},
c_T:function c_T(d){this.a=d},
c_U:function c_U(d){this.a=d},
c_Y:function c_Y(d){this.a=d},
c_Z:function c_Z(d){this.a=d},
c0_:function c0_(){},
c_X:function c_X(d){this.a=d},
c00:function c00(d){this.a=d},
c01:function c01(d){this.a=d},
c_W:function c_W(d){this.a=d},
c02:function c02(d){this.a=d},
cYz(d){var x,w,v,u,t,s,r,q,p=null,o="data:image/jpeg;base64,",n="data:image/png;base64,",m=G.VX(H.ahD(d),"img"),l=A.a([],y.x)
for(x=m.length,w=0;w<m.length;m.length===x||(0,A.S)(m),++w){v=m[w].b.i(0,"src")
if(v!=null)u=C.e.bh(v,o)||C.e.bh(v,n)
else u=!1
if(u){v.toString
if(A.Ml(v,o,0))t=A.ahH(v,o,"",0)
else t=A.ahH(v,n,"",0)
if(C.c.b4(t.length*3,4)>524288){s=C.dC.cp(t)
if(C.e.bh(v,o)){r=D.coi(s)
q=r==null?p:r.mY(0,s,p)
if(q!=null){u=q.a
u=u==null?p:u.a
u=(u==null?0:u)>1200}else u=!1
if(u)l.push(v)}else if(C.e.bh(v,n)){r=D.coi(s)
q=r==null?p:r.mY(0,s,p)
if(q!=null){u=q.a
u=u==null?p:u.a
u=(u==null?0:u)>1000}else u=!1
if(u)l.push(v)}}}}return l},
ar_(d,e,f){var x=0,w=A.m(y.w),v,u,t,s,r,q
var $async$ar_=A.i(function(g,h){if(g===1)return A.j(h,w)
while(true)switch(x){case 0:r=H.ahD(d)
q=new G.JK().a_N(0,r,G.cn3('img[src="'+e+'"]'))
x=q!=null?3:4
break
case 3:u=C.dC.cp(C.e.n(e,"data:image/jpeg;base64,")?C.e.lb(e,"data:image/jpeg;base64,",""):C.e.lb(e,"data:image/png;base64,",""))
x=C.e.bh(e,"data:image/jpeg;base64,")?5:7
break
case 5:x=8
return A.d(D.a09(u,1200,85),$async$ar_)
case 8:t=h
s=C.fH.glF().cp(t)
q.b.m(0,"src","data:image/jpeg;base64,"+s)
x=6
break
case 7:x=C.e.bh(e,"data:image/png;base64,")?9:10
break
case 9:x=D.cLw(u)!=null?11:12
break
case 11:x=13
return A.d(D.arn(u,1000),$async$ar_)
case 13:t=h
s=C.fH.glF().cp(t)
q.b.m(0,"src","data:image/png;base64,"+s)
case 12:case 10:case 6:f.$0()
case 4:v=r.ga5N()
x=1
break
case 1:return A.k(v,w)}})
return A.l($async$ar_,w)},
Pa(a3,a4,a5){var x=0,w=A.m(y.w),v,u,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f,e,d,a0,a1,a2
var $async$Pa=A.i(function(a6,a7){if(a6===1)return A.j(a7,w)
while(true)switch(x){case 0:a0=H.ahD(a3)
a1=H.ahD(a4)
a2=G.VX(a0,"img")
a2=new A.Q(a2,new B.bh_(),A.Z(a2).h("Q<1,h>")).y8(0,new B.bh0())
A.H(a2,!0,a2.$ti.h("x.E"))
u=G.VX(a1,"img")
a2=u.length,t=0
case 3:if(!(t<u.length)){x=5
break}s=u[t]
r=s.b.i(0,"src")
if(r==null||r.length===0){x=4
break}q=J.cyH(r,"data:image/jpeg;base64,")||C.e.bh(r,"data:image/png;base64,")
x=!q?6:7
break
case 6:x=8
return A.d(B.b53(r,a5),$async$Pa)
case 8:if(a7){x=4
break}case 7:x=q?9:11
break
case 9:if(A.Ml(r,"data:image/jpeg;base64,",0))p=A.ahH(r,"data:image/jpeg;base64,","",0)
else p=A.ahH(r,"data:image/png;base64,","",0)
o=C.dC.cp(p)
x=10
break
case 11:x=12
return A.d(B.bgZ(r),$async$Pa)
case 12:n=a7
if(n==null){x=4
break}o=n
case 10:x=o.byteLength>8e5?13:15
break
case 13:m=D.coi(o)
l=m==null?null:m.mY(0,o,null)
x=l!=null?16:18
break
case 16:k=l.a
j=k==null?null:k.a
i=j==null?0:j
j=k==null
h=j?null:k.a
g=i/(h==null?0:h)
k=j?null:J.cSv(k.gaj(k))
f=C.d.fv((k==null?0:k)*g*g)
x=19
return A.d(D.HE(o,i,f>8e5?C.c.b2(C.d.fv(Math.pow(8e5/f,2)*100),1,100):100),$async$Pa)
case 19:e=a7
x=17
break
case 18:e=o
case 17:x=14
break
case 15:e=o
case 14:x=20
return A.d(D.O3(e,a5,null),$async$Pa)
case 20:d=a7
s.b.m(0,"src",d)
case 4:u.length===a2||(0,A.S)(u),++t
x=3
break
case 5:v=a1.ga5N()
x=1
break
case 1:return A.k(v,w)}})
return A.l($async$Pa,w)},
bgZ(d){return B.cYA(d)},
cYA(d){var x=0,w=A.m(y.n),v,u,t,s
var $async$bgZ=A.i(function(e,f){if(e===1)return A.j(f,w)
while(true)switch(x){case 0:s=$.hx().b
s===$&&A.b()
s=s.Q
s===$&&A.b()
u=y.w
x=3
return A.d(s.mv("fetch-http-data",A.z(["targetUrl",d],u,u)),$async$bgZ)
case 3:s=f.a
if(s!=null&&y.B.b(s)){t=J.w(y.B.a(s),"data")
if(typeof t=="string"){v=C.dC.cp(t)
x=1
break}else{A.ek("Error: Data is not a string.")
v=null
x=1
break}}else{A.ek("Error: No data returned from supabase function.")
v=null
x=1
break}case 1:return A.k(v,w)}})
return A.l($async$bgZ,w)},
bh_:function bh_(){},
bh0:function bh0(){},
b53(d,e){var x=0,w=A.m(y.e),v,u
var $async$b53=A.i(function(f,g){if(f===1)return A.j(g,w)
while(true)switch(x){case 0:u=J
x=3
return A.d($.aWa().bB("images").ie(0).cm("link",d).cm("occasion",e),$async$b53)
case 3:v=u.iY(g)
x=1
break
case 1:return A.k(v,w)}})
return A.l($async$b53,w)}},E,W,I,G,K
J=c[1]
A=c[0]
C=c[2]
N=c[173]
O=c[202]
P=c[163]
Q=c[181]
L=c[205]
R=c[314]
S=c[244]
D=c[56]
T=c[198]
U=c[91]
M=c[291]
V=c[245]
F=c[49]
H=c[94]
B=a.updateHolder(c[11],B)
E=c[334]
W=c[260]
I=c[195]
G=c[164]
K=c[199]
B.BI.prototype={
M(){return new B.ab5(A.a([],y.x))}}
B.ab5.prototype={
U(){var x,w=this
w.ah()
x=w.a.c
if(x!=null)w.e=x.i(0,"content")
x=new F.axJ()
x.aiD()
w.as=x
x.PE(new B.c_V(w))
if(w.e==null)w.LP()},
LP(){var x=0,w=A.m(y.v),v=1,u=[],t=this,s,r,q
var $async$LP=A.i(function(d,e){if(d===1){u.push(e)
x=v}while(true)switch(x){case 0:t.v(new B.c_S(t))
v=3
s=t.a.c
s=s==null?null:s.i(0,"load").$0()
x=6
return A.d(y.i.b(s)?s:A.cD(s,y.b),$async$LP)
case 6:s=t.e=e
x=s!=null?7:8
break
case 7:x=9
return A.d(t.rV(s),$async$LP)
case 9:case 8:v=1
x=5
break
case 3:v=2
q=u.pop()
x=5
break
case 2:x=1
break
case 5:t.v(new B.c_T(t))
return A.k(null,w)
case 1:return A.j(u.at(-1),w)}})
return A.l($async$LP,w)},
l(){var x=this.as
x===$&&A.b()
x.l()
this.ag()},
B(d){var x,w,v,u=this,t=null,s=y.u,r=A.a([],s)
if(!u.w&&!u.x){x=u.as
x===$&&A.b()
r.push(new A.cz(C.aK,t,t,new A.cP(new A.ac(0,720,0,1/0),F.cCj(x,""),t),t))}if(u.r)r.push(A.az(t,C.co,C.k,C.ay,t,t,t,t,t,t,t,t,t))
if(u.x){x=A.a([],s)
if(u.y==null)C.b.H(x,A.a([I.bh(E.bCC,t)],s))
else C.b.H(x,A.a([A.az(t,A.al(A.a([I.bh(E.bCR,t),S.dN,U.cDk(t,t,u.y),R.i0,A.U(""+u.z+" / "+u.Q.length,t,t,t,t,t,t,t,M.GD,t,t,t,t,t)],s),C.i,C.f,C.Y,t,C.m),C.k,C.p,t,t,t,t,t,L.b0,t,t,t)],s))
r.push(A.az(t,A.bT(new A.a5(L.b0,A.al(x,C.i,C.f,C.Y,t,C.m),t),t,t),C.k,C.ay,t,t,t,t,t,t,t,t,t))}if(!u.w&&!u.x){x=A.r("Reset",t)
x=F.Xj(u.w?t:new B.c_U(u),x)
w=A.r("Storno",t)
w=F.Xj(u.w?t:u.gbtX(),w)
v=A.r("Save",t)
s=A.az(t,T.dF(Q.rm(A.au(A.a([x,w,F.Xj(u.w?t:u.gaJs(),v)],s),C.i,C.f,C.h,0,t),t),t,C.t,t,t,t,C.aj),C.k,C.hC,t,t,t,t,t,t,t,t,17976931348623157e292)}else s=t
return A.jr(!0,O.f_(t,C.p,new A.ct(C.ai,t,C.af,C.A,r,t),s,t,t,t,!0),!0,C.X,!0,!0)},
uc(){var x=0,w=A.m(y.v),v,u=this,t,s,r,q,p,o
var $async$uc=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:q=u.as
q===$&&A.b()
p=F
o=F
x=3
return A.d(q.JS(),$async$uc)
case 3:t=p.cCk(o.cCl(e))
x=u.a.d!=null?4:5
break
case 4:u.v(new B.c_Y(u))
x=6
return A.d(A.id(C.ep,null,y.b),$async$uc)
case 6:q=u.e
if(q==null)q=""
s=u.a.d
s.toString
x=7
return A.d(B.Pa(q,t,s),$async$uc)
case 7:t=e
s=u.c
s.toString
K.f2(s,!1).f.ef(t)
x=1
break
case 5:u.v(new B.c_Z(u))
q=y.b
x=8
return A.d(A.id(C.ep,null,q),$async$uc)
case 8:s=B.cYz(t)
u.Q=s
x=s.length!==0?9:10
break
case 9:s=u.c
s.toString
x=13
return A.d(N.fr(null,null,!1,null,new B.c0_(),s,null,!0,!0,q),$async$uc)
case 13:x=e?11:12
break
case 11:u.v(new B.c00(u))
x=14
return A.d(A.id(C.N,null,q),$async$uc)
case 14:q=u.Q,s=q.length,r=0
case 15:if(!(r<q.length)){x=17
break}x=18
return A.d(B.ar_(t,q[r],new B.c01(u)),$async$uc)
case 18:t=e
case 16:q.length===s||(0,A.S)(q),++r
x=15
break
case 17:case 12:case 10:u.v(new B.c02(u))
q=u.c
q.toString
K.f2(q,!1).f.ef(t)
case 1:return A.k(v,w)}})
return A.l($async$uc,w)},
a95(){var x=0,w=A.m(y.v),v=this,u
var $async$a95=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:u=v.c
u.toString
K.f2(u,!1).f.ef(null)
return A.k(null,w)}})
return A.l($async$a95,w)},
rV(d){return this.aL9(d)},
aL9(d){var x=0,w=A.m(y.v),v=this,u
var $async$rV=A.i(function(e,f){if(e===1)return A.j(f,w)
while(true)switch(x){case 0:u=v.as
u===$&&A.b()
x=2
return A.d(u.K8(d),$async$rV)
case 2:return A.k(null,w)}})
return A.l($async$rV,w)}}
var z=a.updateTypes(["V<~>()","~()","ft(L)","h(eE)"])
B.c_V.prototype={
$1(d){var x=0,w=A.m(y.F),v,u=this,t,s
var $async$$1=A.i(function(e,f){if(e===1)return A.j(f,w)
while(true)switch(x){case 0:s=u.a
if(s.r)s.f=!0
if(s.f){x=1
break}t=s.e
x=3
return A.d(s.rV(t==null?"":t),$async$$1)
case 3:s.f=!0
case 1:return A.k(v,w)}})
return A.l($async$$1,w)},
$S:88}
B.c_S.prototype={
$0(){this.a.r=!0},
$S:0}
B.c_T.prototype={
$0(){this.a.r=!1},
$S:0}
B.c_U.prototype={
$0(){var x=0,w=A.m(y.v),v=this,u,t
var $async$$0=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:u=v.a
t=u.e
x=2
return A.d(u.rV(t==null?"":t),$async$$0)
case 2:return A.k(null,w)}})
return A.l($async$$0,w)},
$S:1}
B.c_Y.prototype={
$0(){var x=this.a
x.x=x.w=!0},
$S:0}
B.c_Z.prototype={
$0(){var x=this.a
x.x=x.w=!0},
$S:0}
B.c0_.prototype={
$1(d){var x=null
return P.ji(A.a([A.dg(!1,I.bh(V.GH,x),x,x,x,x,x,x,new B.c_X(d),x,x)],y.u),x,E.bDe,x,x,x,E.bCD,x)},
$S:z+2}
B.c_X.prototype={
$0(){return A.bS(this.a,!1).ef(!0)},
$S:0}
B.c00.prototype={
$0(){this.a.y=0},
$S:0}
B.c01.prototype={
$0(){var x=this.a
x.v(new B.c_W(x))},
$S:0}
B.c_W.prototype={
$0(){var x=this.a
x.y=++x.z/x.Q.length},
$S:0}
B.c02.prototype={
$0(){this.a.w=!1},
$S:0}
B.bh_.prototype={
$1(d){var x=d.b.i(0,"src")
return x==null?"":x},
$S:z+3}
B.bh0.prototype={
$1(d){var x
if(d.length!==0)x=!(C.e.bh(d,"data:image/jpeg;base64,")||C.e.bh(d,"data:image/png;base64,"))
else x=!1
return x},
$S:21};(function installTearOffs(){var x=a._instance_0u
var w
x(w=B.ab5.prototype,"gaJs","uc",0)
x(w,"gbtX","a95",1)})();(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.BI,A.I)
x(B.ab5,A.M)
w(A.bI,[B.c_V,B.c0_,B.bh_,B.bh0])
w(A.cn,[B.c_S,B.c_T,B.c_U,B.c_Y,B.c_Z,B.c_X,B.c00,B.c01,B.c_W,B.c02])})()
A.br(b.typeUniverse,JSON.parse('{"BI":{"I":[],"e":[]},"ab5":{"M":["BI"]}}'))
var y=(function rtii(){var x=A.B
return{i:x("V<@>"),x:x("u<h>"),u:x("u<e>"),B:x("A<@,@>"),F:x("aE"),w:x("h"),e:x("y"),b:x("@"),n:x("dH?"),v:x("~")}})();(function constants(){E.bCC=new A.c5("Processing content and detecting large images...",null,W.abo,null,null,null,null,null,null,null,null,null,null,null,null)
E.bCD=new A.c5("Large Images Detected",null,null,null,null,null,null,null,null,null,null,null,null,null,null)
E.bCR=new A.c5("Reducing Images Size...",null,M.GD,null,null,null,null,null,null,null,null,null,null,null,null)
E.bDe=new A.c5("Some images are large and may slow down the app. Press OK to convert them into optimal size.",null,null,null,null,null,null,null,null,null,null,null,null,null,null)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_328",e:"endPart",h:b})})($__dart_deferred_initializers__,"WdUr5Gbf6KCb/z2tUlyLOecMZrI=");