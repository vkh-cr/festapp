((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_85",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={
cxu(d){var x=d.ae(y.o)
x.toString
switch(x.w.a){case 0:x=D.aMM
break
case 1:x=C.f
break
default:x=null}return x},
cdf(d){var x=d.cx,w=A.a_(x)
return new A.eb(new A.aq(x,new B.b_c(),w.h("aq<1>")),new B.b_d(),w.h("eb<1,Z>"))},
cxt(d,e){var x,w,v,u,t=C.b.gN(d),s=B.cde(e,t)
for(x=d.length,w=0;w<d.length;d.length===x||(0,A.N)(d),++w){v=d[w]
u=B.cde(e,v)
if(u<s){s=u
t=v}}return t},
cde(d,e){var x,w,v=d.a,u=e.a
if(v<u){x=d.b
w=e.b
if(x<w)return d.a7(0,new A.m(u,w)).gfZ()
else{w=e.d
if(x>w)return d.a7(0,new A.m(u,w)).gfZ()
else return u-v}}else{u=e.c
if(v>u){x=d.b
w=e.b
if(x<w)return d.a7(0,new A.m(u,w)).gfZ()
else{w=e.d
if(x>w)return d.a7(0,new A.m(u,w)).gfZ()
else return v-u}}else{v=d.b
u=e.b
if(v<u)return u-v
else{u=e.d
if(v>u)return v-u
else return 0}}}},
cdg(d,e){var x,w,v,u,t,s,r,q,p,o,n,m=y.j,l=A.a([d],m)
for(x=e.gW(e);x.q();l=v){w=x.gK(x)
v=A.a([],m)
for(u=l.length,t=w.a,s=w.b,r=w.d,w=w.c,q=0;q<l.length;l.length===u||(0,A.N)(l),++q){p=l[q]
o=p.b
if(o>=s&&p.d<=r){n=p.a
if(n<t)v.push(new A.Z(n,o,n+(t-n),o+(p.d-o)))
n=p.c
if(n>w)v.push(new A.Z(w,o,w+(n-w),o+(p.d-o)))}else{n=p.a
if(n>=t&&p.c<=w){if(o<s)v.push(new A.Z(n,o,n+(p.c-n),o+(s-o)))
o=p.d
if(o>r)v.push(new A.Z(n,r,n+(p.c-n),r+(o-r)))}else v.push(p)}}}return l},
cxs(d,e){var x=d.a,w=!1
if(x>=0)if(x<=e.a){w=d.b
w=w>=0&&w<=e.b}if(w)return d
else return new A.m(Math.min(Math.max(0,x),e.a),Math.min(Math.max(0,d.b),e.b))},
L2:function L2(d,e,f){this.c=d
this.d=e
this.a=f},
b_c:function b_c(){},
b_d:function b_d(){},
Be:function Be(){}},D
A=c[0]
C=c[2]
B=a.updateHolder(c[138],B)
D=c[300]
B.L2.prototype={
A(d){var x=A.aw(d,null,y.h).w,w=x.a,v=w.a,u=w.b,t=B.cxu(d),s=B.cxs(t,w),r=B.cxt(B.cdg(new A.Z(0,0,0+v,0+u),B.cdf(x)),s)
return new A.ah(new A.ar(r.a,r.b,v-r.c,u-r.d),A.Fw(this.d,x.bBC(r)),null)}}
B.Be.prototype={
gwj(){return!1},
gyT(){return!0}}
var z=a.updateTypes([])
B.b_c.prototype={
$1(d){var x=d.gFc(d).gix().zG(0,0)
if(!x)d.gads(d)
return x},
$S:237}
B.b_d.prototype={
$1(d){return d.gFc(d)},
$S:879};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.L2,A.a6)
w(A.bQ,[B.b_c,B.b_d])
x(B.Be,A.f9)})()
A.bD(b.typeUniverse,JSON.parse('{"L2":{"a6":[],"d":[]},"Be":{"f9":["1"],"i_":["1"],"eC":["1"]}}'))
A.mg(b.typeUniverse,JSON.parse('{"Be":1}'))
var y={o:A.y("fm"),j:A.y("q<Z>"),h:A.y("h8")};(function constants(){D.aMM=new A.m(17976931348623157e292,0)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_85",e:"endPart",h:b})})($__dart_deferred_initializers__,"bZsYc3nWznLAR+1Fc7kewtN/xP4=");