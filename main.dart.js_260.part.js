((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_260",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,B={a16:function a16(d,e){this.a=d
this.b=e},
d7e(d){C.b.ec(d,new B.b99())
return d},
yS(d){return B.d7g(d)},
d7g(d){var w=0,v=A.l(x.l),u,t,s,r
var $async$yS=A.f(function(e,f){if(e===1)return A.i(f,v)
while(true)switch(w){case 0:w=d!=null?3:5
break
case 3:t=$.w2().c4("information").i3(0,y.d)
s=A.di()
s.toString
w=6
return A.d(t.cV("occasion",s).adL(0,"type","eq",d),$async$yS)
case 6:r=f
w=4
break
case 5:t=$.w2().c4("information").i3(0,y.d)
s=A.di()
s.toString
w=7
return A.d(t.cV("occasion",s).aHj("type.eq.,type.is.null"),$async$yS)
case 7:r=f
case 4:u=B.d7e(A.aX(J.bO(r,new B.b9i(),x.b),!0,x.c))
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$yS,v)},
b99:function b99(){},
b9i:function b9i(){},
bSz(d,e){var w,v,u,t,s,r,q,p=A.bx("\\d+|\\D+",!0,!1,!1),o=p.vi(0,d),n=x.g
o=A.lZ(o,new B.bSA(),A.t(o).i("y.E"),n)
w=A.E(o,!0,A.t(o).i("y.E"))
o=p.vi(0,e)
n=A.lZ(o,new B.bSB(),A.t(o).i("y.E"),n)
v=A.E(n,!0,A.t(n).i("y.E"))
u=0
while(!0){o=w.length
if(!(u<o&&u<v.length))break
t=w[u]
s=v[u]
o=A.bx("^\\d+$",!0,!1,!1)
if(o.b.test(t)){o=A.bx("^\\d+$",!0,!1,!1)
o=o.b.test(s)}else o=!1
if(o){r=C.c.bt(A.dr(t,null),A.dr(s,null))
if(r!==0)return r}else{if(t===s)q=0
else q=t<s?-1:1
if(q!==0)return q}++u}return C.c.bt(o,v.length)},
bSA:function bSA(){},
bSB:function bSB(){},
apR(){var w=0,v=A.l(x.i),u,t,s,r,q,p
var $async$apR=A.f(function(d,e){if(d===1)return A.i(e,v)
while(true)switch(w){case 0:q=$.tS().c4("occasions").i3(0,"data")
p=A.di()
p.toString
w=3
return A.d(q.cV("id",p).QD(),$async$apR)
case 3:t=e
if(t!=null){s=J.v(t,"data")
if(s!=null&&J.v(s,"game")!=null){q=J.v(s,"game")
p=J.a2(q)
r=p.h(q,"start")!=null?A.eD(p.h(q,"start")):null
u=new B.a16(r,p.h(q,"end")!=null?A.eD(p.h(q,"end")):null)
w=1
break}}u=null
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$apR,v)}}
J=c[1]
A=c[0]
C=c[2]
B=a.updateHolder(c[63],B)
B.a16.prototype={
bn(){var w,v=this.a
v=v==null?null:v.he()
w=this.b
return A.z(["start",v,"end",w==null?null:w.he()],x.g,x.b)}}
var z=a.updateTypes(["Y<B<eA>>([m?])"])
B.b99.prototype={
$2(d,e){var w,v,u=d.w
if(u==null)u=0
w=e.w
v=C.c.bt(u,w==null?0:w)
if(v!==0)return v
u=d.d
u=u==null?null:u.toLowerCase()
if(u==null)u=""
w=e.d
w=w==null?null:w.toLowerCase()
return B.bSz(u,w==null?"":w)},
$S:192}
B.b9i.prototype={
$1(d){return A.au8(d)},
$S:141}
B.bSA.prototype={
$1(d){var w=d.b[0]
w.toString
return w},
$S:422}
B.bSB.prototype={
$1(d){var w=d.b[0]
w.toString
return w},
$S:422};(function installTearOffs(){var w=a.installStaticTearOff
w(B,"duu",0,function(){return[null]},["$1","$0"],["yS",function(){return B.yS(null)}],0,0)})();(function inheritance(){var w=a.inherit,v=a.inheritMany
w(B.a16,A.G)
w(B.b99,A.ep)
v(A.c8,[B.b9i,B.bSA,B.bSB])})()
var y={d:"id,occasion,created_at,updated_at,is_hidden,title,description,order,type,data,information_hidden(*)"}
var x={c:A.D("eA"),l:A.D("B<eA>"),g:A.D("m"),b:A.D("@"),i:A.D("a16?")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_260",e:"endPart",h:b})})($__dart_deferred_initializers__,"YlE0w9K9ayt0RZaei5Fr2DoWkvM=");