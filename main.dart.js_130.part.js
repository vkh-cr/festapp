((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_130",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,B={i9:function i9(d,e,f){this.a=d
this.b=e
this.c=f},ajt:function ajt(d){this.a=d},zt:function zt(){},b43:function b43(){},b44:function b44(){},aBv:function aBv(){}}
J=c[1]
A=c[0]
C=c[2]
B=a.updateHolder(c[117],B)
B.i9.prototype={
dg(d){return new B.i9(this.a.dg(0),this.b,this.c)},
Vw(d){var w
if(d.a.Vw(this.a)){w=this.b
w=w===w&&d.c==this.c}else w=!1
return w},
gaF0(){var w=this.a
return w.gb0(w)*w.gcm(w)*4},
l(){this.a.l()},
j(d){var w=this.c
w=w!=null?w+" ":""
return w+this.a.j(0)+" @ "+A.nY(this.b)+"x"},
gv(d){return A.a3(this.a,this.b,this.c,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a,C.a)},
k(d,e){var w=this
if(e==null)return!1
if(J.ak(e)!==A.N(w))return!1
return e instanceof B.i9&&e.a===w.a&&e.b===w.b&&e.c==w.c}}
B.ajt.prototype={
aLJ(d){++this.a.w},
l(){var w=this.a;--w.w
w.Re()
this.a=null}}
B.zt.prototype={
a8(d,e){var w,v,u,t,s,r,q,p=this
if(p.x)A.V(A.Y(y.a))
p.f=!0
p.a.push(e)
s=p.c
if(s!=null)try{e.a.$2(s.dg(0),!p.r)}catch(r){w=A.ab(r)
v=A.aR(r)
p.axW(A.cK("by a synchronously-called image listener"),w,v)}s=p.d
if(s!=null&&e.c!=null)try{q=e.c
q.toString
q.$2(s.a,s.b)}catch(w){u=A.ab(w)
t=A.aR(w)
if(!J.n(u,p.d.a))A.ff(new A.du(u,t,"image resource service",A.cK("by a synchronously-called image error listener"),null,!1))}},
a7f(){if(this.x)A.V(A.Y(y.a));++this.w
return new B.ajt(this)},
L(d,e){var w,v,u,t,s,r=this
if(r.x)A.V(A.Y(y.a))
for(w=r.a,v=0;v<w.length;++v)if(J.n(w[v],e)){C.b.iw(w,v)
break}if(w.length===0){w=r.y
u=A.a(w.slice(0),A.W(w))
for(t=u.length,s=0;s<u.length;u.length===t||(0,A.M)(u),++s)u[s].$0()
C.b.T(w)
r.Re()}},
Re(){var w,v=this
if(!v.f||v.x||v.a.length!==0||v.w!==0)return
C.b.T(v.b)
w=v.c
if(w!=null)w.a.l()
v.c=null
v.x=!0},
bg_(d){if(this.x)A.V(A.Y(y.a))
this.y.push(d)},
axJ(d){if(this.x)A.V(A.Y(y.a))
C.b.E(this.y,d)},
aaW(d){var w,v,u,t,s,r,q,p,o,n,m=this
if(m.x)A.V(A.Y(y.a))
t=m.c
if(t!=null)t.a.l()
m.c=d
C.b.T(m.b)
t=m.a
if(t.length===0)return
s=A.F(t,!0,x.h)
for(t=s.length,r=d.a,q=d.b,p=d.c,o=0;o<t;++o){w=s[o]
try{w.bu2(new B.i9(r.dg(0),q,p),!1)}catch(n){v=A.ab(n)
u=A.aR(n)
m.axW(A.cK("by an image listener"),v,u)}}},
Gn(d,e,f,g,h){var w,v,u,t,s,r,q,p,o,n,m=this,l="image resource service"
m.d=new A.du(e,h,l,d,f,g)
s=m.a
s=A.F(new A.f9(new A.S(s,new B.b43(),A.W(s).h("S<1,~(z,d4?)?>")),x.m),!0,x.e)
r=m.b
C.b.F(s,r)
C.b.T(r)
w=!1
for(r=s.length,q=0;q<s.length;s.length===r||(0,A.M)(s),++q){v=s[q]
try{v.$2(e,h)
w=!0}catch(p){u=A.ab(p)
t=A.aR(p)
if(!J.n(u,e)){o=A.cK("when reporting an error to an image listener")
n=$.tO
if(n!=null)n.$1(new A.du(u,t,l,o,null,!1))}}}if(!w){s=m.d
s.toString
A.ff(s)}},
axW(d,e,f){return this.Gn(d,e,null,!1,f)},
bxZ(d){var w,v,u,t
if(this.x)A.V(A.Y(y.a))
w=this.a
if(w.length!==0){v=x.i
u=A.F(new A.f9(new A.S(w,new B.b44(),A.W(w).h("S<1,~(n8)?>")),v),!0,v.h("t.E"))
for(w=u.length,t=0;t<w;++t)u[t].$1(d)}}}
B.aBv.prototype={}
var z=a.updateTypes(["~(jh)","~(i9)","~(n8)"])
B.b43.prototype={
$1(d){return d.c},
$S:827}
B.b44.prototype={
$1(d){return d.b},
$S:828};(function aliases(){var w=B.zt.prototype
w.aGx=w.a8
w.aGy=w.L
w.aGw=w.Re})();(function installTearOffs(){var w=a._instance_1i,v=a._instance_1u
var u
w(u=B.zt.prototype,"gTa","a8",0)
v(u,"gaDk","aaW",1)
v(u,"gbxY","bxZ",2)})();(function inheritance(){var w=a.mixin,v=a.inheritMany,u=a.inherit
v(A.z,[B.i9,B.ajt,B.aBv])
u(B.zt,B.aBv)
v(A.c1,[B.b43,B.b44])
w(B.aBv,A.b3)})()
var y={a:"Stream has been disposed.\nAn ImageStream is considered disposed once at least one listener has been added and subsequently all listeners have been removed and no handles are outstanding from the keepAlive method.\nTo resolve this error, maintain at least one listener on the stream, or create an ImageStreamCompleterHandle from the keepAlive method, or create a new stream for the image."}
var x={h:A.A("jh"),m:A.A("f9<~(z,d4?)>"),i:A.A("f9<~(n8)>"),e:A.A("~(z,d4?)")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_130",e:"endPart",h:b})})($__dart_deferred_initializers__,"ZwQqApuq+lwPleimH80sjjPCed0=");