((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,D,E,B={b6t:function b6t(d,e,f,g,h,i,j){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j},
eSP(d,e,f,g){return new B.abs(g,f,e)},
abs:function abs(d,e,f){this.d=d
this.e=e
this.a=f},
bxZ:function bxZ(){this.c=this.a=null},
aWJ(d){var x=0,w=A.p(y.f),v
var $async$aWJ=A.l(function(e,f){if(e===1)return A.m(f,w)
for(;;)switch(x){case 0:x=2
return A.f($.e3().gdL().Nu(d),$async$aWJ)
case 2:v=f.a
x=v!=null?3:4
break
case 3:x=5
return A.f(C.ne.MW(0,"refresh",J.an(v.e)),$async$aWJ)
case 5:case 4:return A.n(null,w)}})
return A.o($async$aWJ,w)}}
J=c[1]
A=c[0]
C=c[2]
D=c[174]
E=c[57]
B=a.updateHolder(c[32],B)
B.b6t.prototype={}
B.abs.prototype={
L(){return new B.bxZ()}}
B.bxZ.prototype={
S(){this.a_()
this.wf()},
wf(){var x=0,w=A.p(y.f),v=1,u=[],t=this,s,r,q,p,o,n,m,l,k
var $async$wf=A.l(function(d,e){if(d===1){u.push(e)
x=v}for(;;)switch(x){case 0:A.cx().$1("TransferPage: Handling Session...")
s=$.e3().gdL().c!=null
x=!s?2:3
break
case 2:x=4
return A.f(A.WL(),$async$wf)
case 4:s=e
case 3:if(!s){p=t.a.d
p=p!=null&&p.length!==0}else p=!1
x=p?5:6
break
case 5:v=8
A.cx().$1("TransferPage: Setting session from token...")
p=$.Mc().b
p===$&&A.b()
x=p.gdL().c!=null?11:12
break
case 11:x=13
return A.f(p.gdL().w6(),$async$wf)
case 13:case 12:p=t.a.d
p.toString
x=14
return A.f(B.aWJ(p),$async$wf)
case 14:x=15
return A.f(A.alf(),$async$wf)
case 15:s=!0
v=1
x=10
break
case 8:v=7
l=u.pop()
r=A.ag(l)
A.cx().$1("TransferPage: Session set error: "+A.t(r))
x=16
return A.f(A.WL(),$async$wf)
case 16:n=e
s=n
x=10
break
case 7:x=1
break
case 10:case 6:x=s?17:19
break
case 17:x=t.c!=null?20:21
break
case 20:v=23
A.cx().$1("TransferPage: Session valid ("+A.t(s)+"). calling handlePostLoginNavigation")
p=t.c
p.toString
m=t.a.e
x=26
return A.f(E.a5Q(p,m==null?"/":m,!0),$async$wf)
case 26:v=1
x=25
break
case 23:v=22
k=u.pop()
q=A.ag(k)
A.cx().$1("TransferPage: Smart nav failed. Error: "+A.t(q))
p=t.c
x=p!=null?27:28
break
case 27:x=29
return A.f(D.f8(p,!1).f.UI("/",y.a),$async$wf)
case 29:case 28:x=25
break
case 22:x=1
break
case 25:case 21:x=18
break
case 19:A.cx().$1("TransferPage: Session INVALID ("+A.t(s)+"). Redirecting to LoginRoute.")
p=t.c
if(p!=null){p=D.f8(p,!1).f
p.amN(0,new B.b6t("LoginRoute",null,C.c7,C.c7,null,"",null),y.a)}case 18:return A.n(null,w)
case 1:return A.m(u.at(-1),w)}})
return A.o($async$wf,w)},
A(d){return C.oW}}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(B.b6t,A.NZ)
x(B.abs,A.N)
x(B.bxZ,A.Q)})()
A.cg(b.typeUniverse,JSON.parse('{"b6t":{"NZ":["~"]},"abs":{"N":[],"k":[]},"bxZ":{"Q":["abs"]}}'))
var y={a:A.L("M?"),f:A.L("~")}};
(a=>{a["l107wxEwlxilQ4uOhrUNPr41wtY="]=a.current})($__dart_deferred_initializers__);