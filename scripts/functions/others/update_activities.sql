...
DECLARE
    ...
    db_activity_ids BIGINT[];
    incoming_activity_ids BIGINT[] := ARRAY[]::BIGINT[];
    activity_id_to_check BIGINT;
...
BEGIN
    ...
    FOR activity_json IN SELECT * FROM jsonb_array_elements(p_activities_data) LOOP
        IF activity_json.value->>'id' IS NOT NULL THEN
            incoming_activity_ids := array_append(incoming_activity_ids, (activity_json.value->>'id')::BIGINT);
        END IF;
    END LOOP;
    ...
    IF array_length(db_activity_ids, 1) > 0 THEN
        FOREACH activity_id_to_check IN ARRAY db_activity_ids LOOP
            IF NOT (activity_id_to_check = ANY(incoming_activity_ids)) THEN
                -- Cascading deletes should handle related activity_assignments,
                -- and then activity_assignment_places/events for those assignments.
    ...
    FOR activity_json IN SELECT * FROM jsonb_to_recordset(p_activities_data) AS act(
        ...
        data JSONB,
        unit BIGINT,
        assignments JSONB -- No longer linked_place_ids or linked_event_ids here
    )
    LOOP
        -- Upsert Activity
        IF activity_json.id IS NOT NULL THEN
            UPDATE public.activities SET
                title = activity_json.title, description = activity_json.description, type = activity_json.type,
                occasion = p_occasion_id, unit = activity_json.unit, is_hidden = activity_json.is_hidden,
                "order" = activity_json."order", data = activity_json.data, updated_at = now()
            WHERE id = activity_json.id
            RETURNING activities.id INTO v_activity_id;
        ELSE
            INSERT INTO public.activities (
                title, description, type, occasion, unit, is_hidden, "order", data, created_at, updated_at
            ) VALUES (
                activity_json.title, activity_json.description, activity_json.type, p_occasion_id,
                activity_json.unit, activity_json.is_hidden, activity_json."order", activity_json.data, now(), now()
            ) RETURNING id INTO v_activity_id;
        END IF;

        -- REMOVED: Management of activity_assignment_places and activity_assignment_events directly under activity.
        -- This is now handled per assignment.
        ...
        FOR db_assignment_id_to_check IN SELECT id FROM public.activity_assignments WHERE activity_id = v_activity_id LOOP
            IF NOT (db_assignment_id_to_check = ANY(incoming_assignment_ids_for_activity)) THEN
                -- Cascading delete on activity_assignments table should remove related
                -- activity_assignment_places and activity_assignment_events entries.
                DELETE FROM public.activity_assignments WHERE id = db_assignment_id_to_check;
            END IF;
        END LOOP;

        -- Upsert assignments and their related places/events
        IF jsonb_typeof(activity_json.assignments) = 'array' AND jsonb_array_length(activity_json.assignments) > 0 THEN
            FOR assignment_json_record IN SELECT * FROM jsonb_to_recordset(activity_json.assignments) AS ass(
                id TEXT, -- Assignment's own UUID
                "user" TEXT, -- User UUID string
            ...
            LOOP
                BEGIN
                    ...
                    ELSE
                        user_uuid := NULL; -- Allow null users in assignments if schema permits
                    END IF;
                EXCEPTION
                ...
                -- Manage activity_assignment_places links for this assignment
                ...
                -- Manage activity_assignment_events links for this assignment
