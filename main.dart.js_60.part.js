((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_60",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,F,G,H,I,K,L,M,N,O,P,Q,B={
d2J(){return new B.DI(null)},
DI:function DI(d){this.a=d},
aQd:function aQd(d,e){var _=this
_.e=_.d=!1
_.r=_.f=null
_.w=d
_.x=e
_.c=_.a=null},
cd4:function cd4(d){this.a=d},
cd5:function cd5(d,e){this.a=d
this.b=e},
cd1:function cd1(d){this.a=d},
cd2:function cd2(d){this.a=d},
cd3:function cd3(d){this.a=d},
aZ4(d){var x=0,w=A.m(y.y),v,u
var $async$aZ4=A.i(function(e,f){if(e===1)return A.j(f,w)
while(true)switch(x){case 0:d.m(0,"organization",1)
u=$.e1().Q
u===$&&A.b()
x=3
return A.d(u.mv("register",d),$async$aZ4)
case 3:v=f.a
x=1
break
case 1:return A.k(v,w)}})
return A.l($async$aZ4,w)}},D,R,E,S,T
J=c[1]
A=c[0]
C=c[2]
F=c[104]
G=c[202]
H=c[193]
I=c[252]
K=c[169]
L=c[198]
M=c[197]
N=c[214]
O=c[124]
P=c[157]
Q=c[221]
B=a.updateHolder(c[26],B)
D=c[330]
R=c[73]
E=c[33]
S=c[195]
T=c[266]
B.DI.prototype={
M(){var x=null
return new B.aQd(A.a([A.w_(x,x,x,x,1,x,!0,x,x,x,x,x,x,"name"),A.w_(x,x,x,x,2,x,!0,x,x,x,x,x,x,"surname"),A.w_(x,x,x,x,3,x,x,x,x,x,x,x,x,"sex"),A.w_(x,x,x,x,4,x,!0,x,x,x,x,x,x,"email"),A.w_(x,x,x,x,5,x,!0,x,x,x,x,x,x,"city"),A.w_(x,x,x,x,6,x,x,x,x,x,x,x,x,"birth_year")],y.a),new A.aN(x,y.c))}}
B.aQd.prototype={
l(){this.ag()},
B(d){var x,w,v,u,t,s=this,r=null,q=A.cBZ(r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r)
q.ay=s.w
x=E.cBX(q)
s.r=x
w=s.x
x.a=new E.aq9(r,r,w,r,r,r,r)
x=S.bh(D.bCo,r)
x=H.i8(r,r,r,!0,r,r,1,!0,r,r,!1,r,!1,r,r,K.tB(new B.cd4(d)),r,!0,r,r,r,r,r,x,r,r,r,1,r)
if(s.e){w=A.aC(r,r,A.D(d).ax.a===C.u?$.dK():C.r,r,r,r,r,r,r,r,r,18,r,r,r,r,r,!0,r,r,r,r,r,r,r,r)
v=s.f
v=v==null?r:v.i(0,"email")
u=y.w
u=new A.a5(D.aq4,A.zb(r,r,r,C.c2,r,r,!0,r,A.dd(A.a([A.dd(r,r,r,w,A.r("Almost done! Your credentials for signing in to the app have been sent to your email {email}. Please check your inbox to complete the registration.",A.z(["email",v],u,u))),D.bHa],y.F),r,r,r,r),C.b6,r,r,C.a1,C.aG),r)
w=u}else{v=s.r
v=A.H(E.cBU(d,v.a.c,v),!0,y.u)
v.push(N.a3)
u=s.d?r:new B.cd5(s,d)
t=A.r("Sign up",r)
v.push(O.N0(C.cC,d,50,!s.d,t,u,C.p,250))
w=R.bdi(new F.xI(A.al(v,C.i,C.f,C.h,r,C.m),r),w,r)}return G.f_(x,r,new A.cz(C.aK,r,r,new A.cP(new A.ac(0,720,0,1/0),L.dF(new A.a5(Q.bi,w,r),r,C.t,r,r,r,C.I),r),r),r,r,r,r,r)}}
var z=a.updateTypes([])
B.cd4.prototype={
$0(){return P.wE(this.a)},
$S:0}
B.cd5.prototype={
$0(){var x=0,w=A.m(y.q),v=this,u,t,s,r,q,p
var $async$$0=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:F.a6B(!0)
u=v.a
t=u.x.gX()
t=t==null?null:t.afN()
x=t===!0?2:3
break
case 2:u.v(new B.cd1(u))
t=u.r
t.toString
s=y.w
t=A.jn(E.cBV(t,!0).i(0,"fields"),s,y.b)
u.f=t
r=v.b
q=r.ac(y.v)
if(q==null)q=null
else{q=q.r.a
q===$&&A.b()
q=q.mM("_")}t.m(0,"lang",q)
q=u.f
q.toString
x=4
return A.d(B.aZ4(q),$async$$0)
case 4:p=e
t=J.a4(p)
if(J.o(t.i(p,"code"),200)){A.bF(r,A.r("Registration is almost complete!",null),C.a8)
u.v(new B.cd2(u))}else if(J.o(t.i(p,"code"),409))A.bF(r,A.r("Registration failed: Email {email} is already in use.",A.z(["email",t.i(p,"email")],s,s)),C.bf)
else A.bF(r,A.r("Registration has failed.",null),C.bf)
u.v(new B.cd3(u))
case 3:return A.k(null,w)}})
return A.l($async$$0,w)},
$S:1}
B.cd1.prototype={
$0(){this.a.d=!0},
$S:0}
B.cd2.prototype={
$0(){this.a.e=!0},
$S:0}
B.cd3.prototype={
$0(){this.a.d=!1},
$S:0};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.DI,A.I)
x(B.aQd,A.M)
w(A.cn,[B.cd4,B.cd5,B.cd1,B.cd2,B.cd3])})()
A.br(b.typeUniverse,JSON.parse('{"DI":{"I":[],"e":[]},"aQd":{"M":["DI"]}}'))
var y={a:A.B("u<iL>"),F:A.B("u<is>"),c:A.B("aN<vZ>"),y:A.B("A<h,@>"),w:A.B("h"),u:A.B("e"),v:A.B("kH"),b:A.B("@"),q:A.B("~")};(function constants(){D.aq4=new A.ap(12,88,12,12)
D.bCo=new A.c5("Sign up",null,null,null,null,null,null,null,null,null,null,null,null,null,null)
D.bqk=new A.a5(T.La,I.Mu,null)
D.bHa=new M.ms(D.bqk,C.eF,null,null)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_60",e:"endPart",h:b})})($__dart_deferred_initializers__,"QxxfKIAQ8fv4fEj/tBX6Rcz2AOg=");