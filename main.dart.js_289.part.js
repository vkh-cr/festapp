((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_289",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,E,B={
cXM(d,e){return new B.BC(e)},
BC:function BC(d){this.a=d},
aKt:function aKt(d,e,f){var _=this
_.d=$
_.e=d
_.ft$=e
_.bu$=f
_.c=_.a=null},
bYh:function bYh(){},
bYi:function bYi(){},
aU0:function aU0(){}},F,C,D,G,H
A=c[0]
E=c[202]
B=a.updateHolder(c[8],B)
F=c[86]
C=c[32]
D=c[66]
G=c[203]
H=c[172]
B.BC.prototype={
M(){var x=null,w=A.a(["Orders"],y.h)
if(A.hd("ticket",x))w.push("Tickets")
if(A.hd("blueprint",x))w.push("Blueprint")
w.push("Form")
w.push("Report")
w.push("Email Templates")
w.push("Users")
return new B.aKt(w,x,x)}}
B.aKt.prototype={
aT(){var x=0,w=A.m(y.f),v=this,u
var $async$aT=A.i(function(d,e){if(d===1)return A.j(e,w)
while(true)switch(x){case 0:x=$.e1().gdD().d==null?2:3
break
case 2:u=v.c
u.toString
x=4
return A.d(H.lC(u,"login",y.a),$async$aT)
case 4:case 3:v.c8()
return A.k(null,w)}})
return A.l($async$aT,w)},
U(){var x=this
x.ah()
x.d=F.DW(null,0,x.e.length,x)},
B(d){var x,w,v=null,u=this.e,t=A.Z(u).h("Q<1,hK>"),s=A.H(new A.Q(u,new B.bYh(),t),!0,t.h("ab.E"))
t=this.d
t===$&&A.b()
u=C.cyO(d,s,t)
x=this.d
w=A.Z(s).h("Q<1,e>")
return D.Gw(E.f_(u,v,D.Ke(A.H(new A.Q(s,new B.bYi(),w),!0,w.h("ab.E")),x,G.dK),v,v,v,v,v),0,t.c)}}
B.aU0.prototype={
l(){var x=this,w=x.bu$
if(w!=null)w.N(0,x.gho())
x.bu$=null
x.ag()},
cd(){this.dk()
this.d8()
this.hp()}}
var z=a.updateTypes(["hK(h)","e(hK)"])
B.bYh.prototype={
$1(d){var x=C.cyP().i(0,d)
x.toString
return x},
$S:z+0}
B.bYi.prototype={
$1(d){return d.c},
$S:z+1};(function inheritance(){var x=a.mixinHard,w=a.inherit,v=a.inheritMany
w(B.BC,A.I)
w(B.aU0,A.M)
w(B.aKt,B.aU0)
v(A.bI,[B.bYh,B.bYi])
x(B.aU0,A.fo)})()
A.br(b.typeUniverse,JSON.parse('{"BC":{"I":[],"e":[]},"aKt":{"M":["BC"]}}'))
var y={h:A.B("u<h>"),a:A.B("G?"),f:A.B("~")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_289",e:"endPart",h:b})})($__dart_deferred_initializers__,"NJ7dGqDsT5Ri6fVBRcp9CT9dass=");