...
export function parseEmail(
  ...
  if (provider === "fio_email") {
    ...
    const amountMatch = body.match(
      /(?:Částka|Castka|Amount):\s*([\d\s,.-]+)\s*([A-Z]{3})/i,
    );
    ...
    if (amountMatch) {
      ...
      const lookedUpBankName = counterpartyBankCode
        ? (BANK_CODES[counterpartyBankCode] || null)
        : null;
      ...
      return {
        ...
        movementId: null,
        bankCommandId: idMatch ? idMatch[1] : null,
        payerReference: null,
      };
    }
  }
  ...
  if (provider === "airbank_email") {
    ...
    const idMatch = body.match(
      /(?:Kód transakce|Kod transakce)\s*:\s*([0-9]+)/i,
    );

    // Date
    const dateMatch = body.match(
      /(?:Datum zaúčtování|Datum zauctovani)\s*:\s*(\d{2}\.\d{2}\.\d{4})/i,
    );
    ...
    if (amountMatch) {
      ...
      return {
        ...
        message: msgMatch ? msgMatch[1].trim() : null,
        date: isoDate,
        movementId: idMatch ? idMatch[1] : null,
        bankCommandId: null,
        payerReference: null,
      };
    }
