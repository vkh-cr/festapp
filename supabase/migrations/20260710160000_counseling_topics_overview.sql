-- Migration: add get_counseling_topics_overview (per-topic counseling
-- availability for the rozcestník, so the client can grey out empty areas).
-- Additive: one new SECURITY DEFINER function, gated on the counseling feature.

CREATE OR REPLACE FUNCTION get_counseling_topics_overview(p_occasion BIGINT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
