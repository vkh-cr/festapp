((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_199",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,D,C,B={
c7u(d){var x=y.u
return C.c0r(C.c0r(d,new B.aTT(),x,y.w),new B.aTU(),x,y.f)},
aTT:function aTT(){},
aTU:function aTU(){},
pl(d){var x=0,w=A.l(y.u),v,u,t,s,r,q,p,o,n,m,l,k
var $async$pl=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:s=$.fB()
n=A
x=3
return A.c(s.bV("events").hq(0,"id,updated_at,title,start_time,end_time,max_participants,split_for_men_women,is_group_event,is_hidden,type,places(id, title),event_groups!event_groups_event_parent_fkey(event_child)").cr("id",d).lx(0),$async$pl)
case 3:r=n.ps(f)
q=y.u
x=4
return A.c(A.pK(D.c.k(d),A.ze(),"events",q),$async$pl)
case 4:p=f
o=p==null
if(o)u=null
else{u=p.c
u.toString
t=r.c
t.toString
t=u.mC(t)
u=t}x=u!==!1?5:7
break
case 5:o=r.b
o.toString
n=r
m=J
x=8
return A.c(A.ag8(A.a([o],y.r)),$async$pl)
case 8:n.at=m.u(f,0).at
x=6
break
case 7:r.at=o?null:p.at
case 6:o=$.dZ()
u=r.b
x=o.gd8().d!=null?9:11
break
case 9:u.toString
n=r
x=12
return A.c(B.aUK(u),$async$pl)
case 12:n.cx=f
n=r
x=13
return A.c(B.aUJ(u),$async$pl)
case 13:n.ax=f
x=10
break
case 11:u.toString
n=r
x=14
return A.c(B.aoh(u),$async$pl)
case 14:n.cx=f
case 10:x=r.y!=null?15:16
break
case 15:s=s.bV("events").hq(0,"id,title,start_time,end_time,max_participants,event_users(count)")
u=r.y
u.toString
n=r
m=B
l=A
k=J
x=17
return A.c(s.Bt("id",u).cr("is_hidden",!1),$async$pl)
case 17:n.r=m.c7u(l.bj(k.ch(f,new B.aUC(),y.b),!0,q))
x=o.gd8().d!=null?18:19
break
case 18:x=20
return A.c(E.ag9(r.r),$async$pl)
case 20:case 19:case 16:if(r.ch&&F.bZQ())r.CW=!0
v=r
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$pl,w)},
aUJ(d){var x=0,w=A.l(y.e),v
var $async$aUJ=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:x=3
return A.c($.fB().bV("event_users").i4(0).cr("event",d).cr("user",$.dZ().gd8().c.a).a5v(0),$async$aUJ)
case 3:v=f.b>0
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$aUJ,w)},
aUK(d){var x=0,w=A.l(y.e),v
var $async$aUK=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:x=3
return A.c($.fB().bV("event_users_saved").i4(0).cr("event",d).cr("user",$.dZ().gd8().c.a).G4(),$async$aUK)
case 3:v=f!=null
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$aUK,w)},
aUC:function aUC(){},
aoh(d){var x=0,w=A.l(y.e),v,u
var $async$aoh=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:u=J
x=3
return A.c(A.Fg(),$async$aoh)
case 3:v=u.mU(f,d)
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$aoh,w)}},E,F
J=c[1]
A=c[0]
D=c[2]
C=c[72]
B=a.updateHolder(c[61],B)
E=c[100]
F=c[108]
var z=a.updateTypes([])
B.aTT.prototype={
$1(d){var x=d.as
return x==null?"":x},
$S:870}
B.aTU.prototype={
$1(d){return d.cy},
$S:871}
B.aUC.prototype={
$1(d){return A.ps(d)},
$S:51};(function inheritance(){var x=a.inheritMany
x(A.c0,[B.aTT,B.aTU,B.aUC])})()
var y={f:A.y("bz"),u:A.y("cZ"),r:A.y("p<q>"),w:A.y("f"),e:A.y("v"),b:A.y("@")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_199",e:"endPart",h:b})})($__dart_deferred_initializers__,"iU/fr70ME2t12BfRAZBHZUVW+No=");